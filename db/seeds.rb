# frozen_string_literal: true
# rubocop:disable Layout/LineLength

# This file should contain all the record creation needed to seed the database with its default values.
# The data can then be loaded with the rails db:seed command (or created alongside the database with db:setup).
#
# Examples:
#
#   movies = Movie.create([{ name: 'Star Wars' }, { name: 'Lord of the Rings' }])
#   Character.create(name: 'Luke', movie: movies.first)

Teacher.create!(
  name: 'Carolina Galvez',
  image: 'https://raw.githubusercontent.com/nriqu322/teachers-wise-api/main/public/images/teacher1.jpg',
  description: 'I have 4 years of experience and I\'m a specialist in Early Childhood Special Education.'
)

Teacher.create!(
  name: 'Kenneth Peterson',
  image: 'https://raw.githubusercontent.com/nriqu322/teachers-wise-api/main/public/images/teacher2.jpg',
  description: 'I have 6 years of experience and I\'m a specialist in Applied Behavior Analysis.'
)

Teacher.create!(
  name: 'Fatima Sani',
  image: 'https://raw.githubusercontent.com/nriqu322/teachers-wise-api/main/public/images/teacher3.jpg',
  description: 'I have 2 years of experience and I\'m a specialist in Visual and/or Hearing Impairment.'
)

Teacher.create!(
  name: 'Van Nam Nguyen',
  image: 'https://raw.githubusercontent.com/nriqu322/teachers-wise-api/main/public/images/teacher4.jpg',
  description: 'I have 3 years of experience and I\'m a specialist in English as a Second Language.'
)

Teacher.create!(
  name: 'Karen Williams',
  image: 'https://raw.githubusercontent.com/nriqu322/teachers-wise-api/main/public/images/teacher5.jpg',
  description: 'I have 3 years of experience and I\'m a specialist in Early Childhood Special Education.'
)

Teacher.create!(
  name: 'Charles Bernard',
  image: 'https://raw.githubusercontent.com/nriqu322/teachers-wise-api/main/public/images/teacher6.jpg',
  description: 'I have 2 years of experience and I\'m a specialist in English as a Second Language.'
)

# rubocop:enable Layout/LineLength
